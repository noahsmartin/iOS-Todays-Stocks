...
@property bool open;
@property bool marketClose;
@property bool marketOpen;
@property bool change;
@property bool changePercent;
@property bool realTimePrice;
...
-(void)startRequestWithSymbol:(NSString *)symbol duration:(DURATION)duration withBlock:(void (^)(StockGraph *))block {
    ...
    body = [body stringByAppendingString:[NSString stringWithFormat:@"<query id=\"0\" timestamp=\"%f\" type=\"getchart\">", [[NSDate date] timeIntervalSince1970]]];
...
- (void)parser:(NSXMLParser *)parser foundCharacters:(NSString *)string
{
    ...
    } else if(self.marketClose) {
        [self.graph setMarketClose:[string longLongValue]];
    } else if(self.marketOpen) {
        [self.graph setMarketOpen:[string longLongValue]];
    } else if(self.realTimePrice) {
        self.graph.realTimePrice = [string doubleValue];
    } else if(self.change) {
...
- (void)parser:(NSXMLParser *)parser didStartElement:(NSString *)elementName namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qualifiedName attributes:(NSDictionary *)attributeDict {
    if([elementName isEqualToString:@"open"]) {
        ...
        self.marketClose = YES;
    } else if([elementName isEqualToString:@"marketopen"]) {
        self.marketOpen = YES;
    } else if([elementName isEqualToString:@"realtimeprice"]) {
        self.realTimePrice = YES;
